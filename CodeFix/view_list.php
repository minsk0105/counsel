...
<?php
    ...
    if (isset($_GET['name'])) {
        ...
        $find_sql = "SELECT * FROM nons_counsel INNER JOIN nons_file
            ON nons_counsel.register_time = nons_file.upload_time
        ";

        $name_sql ="SELECT name_origin, upload_time FROM nons_file";
        ...
        while ($row = mysqli_fetch_array($query)) {
            ...
            if ($phone = $row['phone']) {
                ...
                if (in_array($row['register_time'], $file_isset)) {
                    $count = $sum++;
                    $allowed_fileName = substr($file_name[$count], strrpos($file_name[$count], '.') + 1);
                    $img_src = '';
